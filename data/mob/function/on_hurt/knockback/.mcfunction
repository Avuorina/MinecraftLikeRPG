    ...
    # 既に回転は引き継がれているので、Position変更のみ行う
        ...
        # AECの位置を取得
            ...
            # Debug: AEC召喚確認
                ...
                #AECのPositon取得
            # Debug: Raw Data
                #data get storage mob:temp Pos

            # いつものノックバックはY軸方向は固定なので上書き
                data modify storage mob:temp Pos[1] set value 0.32
                execute store result score $KnockbackX _ run data get storage mob:temp Pos[0] 1000
                execute store result score $KnockbackY _ run data get storage mob:temp Pos[1] 1000
                execute store result score $KnockbackZ _ run data get storage mob:temp Pos[2] 1000
            ...
            # Motionの適用
                execute store result storage mob:temp Pos[0] double 0.00001 run scoreboard players get $KnockbackX _
                execute store result storage mob:temp Pos[1] double 0.00001 run scoreboard players get $KnockbackY _
                execute store result storage mob:temp Pos[2] double 0.00001 run scoreboard players get $KnockbackZ _
