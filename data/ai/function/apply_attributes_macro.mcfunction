...
# $(ai_speed) は "入力値 - 1.0" (例: 1.2倍なら 0.2)
    $attribute @s minecraft:generic.movement_speed modifier add rpg:ai_speed "AI Speed" $(ai_speed) add_multiplied_base

# 2. 索敵範囲 (Follow Range) - 倍率加算 (add_multiplied_base)
    $attribute @s minecraft:generic.follow_range modifier add rpg:ai_follow_range "AI Follow Range" $(ai_follow_range) add_multiplied_base
...
# 一旦 base set を modifier add ... add_value に変えるのが行儀が良い。
    $attribute @s minecraft:generic.knockback_resistance modifier add rpg:ai_kb_resist "AI KB Resist" $(ai_knockback_resistance) add_value
