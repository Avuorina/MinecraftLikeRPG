...
# Yaw (Horizontal)
    ...
    # Absoluteフラグがある場合は0リセット（絶対指定）
        execute if data storage rpg_skill: data.Absolute run scoreboard players set #calc _ 0

    execute store result score #val _ run data get storage rpg_skill: data.Direction[0] 100
...
execute store result score #val _ run data get storage rpg_skill: data.Direction[1] 100
scoreboard players operation #calc _ += #val _
