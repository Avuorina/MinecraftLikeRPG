...
# execute unless data storage bank:mob Name run
    ...
    # execute unless data storage bank:mob FollowRange run
        ...
        # 武器防具
            ...
            # ステータス読み込み & 補正適用
            # MaxHP
                execute store result score @s MaxHP run data get storage bank:mob Status.HPMax
        ...
        # LVスコアをStorageに保存してマクロ呼び出し
            ...
            function bank_manager:mob/summon/apply_status/level/ with storage bank:mob
