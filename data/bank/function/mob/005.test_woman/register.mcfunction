# テストウーマン データ登録
# ID: 005.test_woman
# bank:mob/005.test_woman/register
...
# ベースエンティティ (summonに使うのでStorageへのベース保存は不要だが、参照用に残しても良い)
    data modify storage rpg_mob: "ベース" set value {id:"minecraft:villager",Tags:[mob.005.test_woman,Init,Global,Debug,Blow,TestWoman,FRIENDLY]}
...
# ユーザー検証: シングルクォート無し (Compound Tag) で保存しても動く
    data modify storage rpg_mob: BaseNameJSON set value {"color":"green","text":"テストウーマン"}
...
# 装備 (個別設定)
    ...
    data modify storage rpg_mob: "見た目".ArmorItems[3] merge value {id:"minecraft:command_block",Count:1b}
